            ...
            try {
                ...
                if (!companyData) {
                    ...
                    if (user) {
                        ...
                        setOtherComments(commentsData.filter(c => c.id !== myReview?.id && c.status === 'approved'));
                    } else {
                        setMyComment(undefined);
                        setOtherComments(commentsData.filter(c => c.status === 'approved'));
                    }
                }
            } catch (err) {
                            ...
                            <div className="bg-[rgba(253,253,253,1)] shadow-[0px_6px_10px_rgba(177,177,177,0.08)] border border flex flex-col items-stretch text-xs font-medium mr-[43px] mt-[5px] py-[23px] rounded-[26px] border-solid max-md:max-w-full max-md:mr-2.5">
                                <div className="flex w-full flex-col text-[#6B6B6E] px-[30px] max-md:max-w-full max-md:px-5">
                                    <div className="flex items-stretch gap-[5px] text-xl text-[#202023] font-semibold leading-[1.4]">
                                        <img
                                            src={myComment.authorAvatar || "https://api.builder.io/api/v1/image/assets/TEMP/ed4d506d869550e63301cda115d2a37f3f3d8102?placeholderIfAbsent=true"}
                                            alt="User avatar"
                                        ...
                                        <div className="flex items-center gap-1 ml-auto">
                                            <div className="flex">
                                                {[1, 2, 3, 4, 5].map((star) => (
                                                    <svg key={star} width="16" height="16" viewBox="0 0 24 24" fill={star <= myComment.rating ? "#FFD700" : "#E5E7EB"}>
                                    ...
                                    <div className="flex items-stretch gap-2.5 text-[9px] mt-[29px]">
                                        <div className="text-[#6B6B6E] my-auto">
                                            Yararlı <span className="font-semibold">{myComment.likesCount}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="mr-[43px] mt-[30px] max-md:max-w-full max-md:mr-2.5">
                            <div className="gap-5 flex max-md:flex-col max-md:items-stretch">
                                {otherComments.slice(0, 2).map((comment) => (
                                    <div key={comment.id} className="w-6/12 max-md:w-full max-md:ml-0">
                                        <ReviewCard
                                            author={comment.authorName}
                                            date={formatDate(comment.date)}
                                            content={comment.message}
                                            helpful={comment.likesCount}
                                            avatar={comment.authorAvatar || "https://api.builder.io/api/v1/image/assets/TEMP/ed4d506d869550e63301cda115d2a37f3f3d8102?placeholderIfAbsent=true"}
                                        />
                                    </div>
                                ))}
                                                            ...
                                                            <article key={review.id} className="w-full">
                                                                <div className="flex w-full items-stretch gap-[40px_65px]">
                                                                    <div className="flex items-stretch gap-[13px] grow shrink basis-auto">
                                                                        <img
                                                                            src={review.authorAvatar || "https://api.builder.io/api/v1/image/assets/TEMP/ed4d506d869550e63301cda115d2a37f3f3d8102?placeholderIfAbsent=true"}
