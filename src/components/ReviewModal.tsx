...
export const ReviewModal: React.FC<ReviewModalProps> = ({ isOpen, onClose, company }) => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'Enter') {
                if (step === 1) {
                    ...
                    setStep(4);
                } else if (step === 4 && description.trim() && termsAccepted && !isSubmitting) {
                    handleSubmit();
                } else if (step === 5) {
                    onClose();
                }
    ...
    const maskName = (name: string) => {
        if (!name) return '***';
        return name.split(' ').map(part => part.substring(0, 2) + '***').join(' ');
    };
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-start justify-between mb-8">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="w-16 h-16 rounded-full bg-black flex items-center justify-center shrink-0">
                                {company.imageUrl ? (
                                    <img src={company.imageUrl} alt={company.name} className="w-full h-full object-cover rounded-full" />
                                ) : (
                                    <div className="w-full h-full rounded-full bg-gradient-to-br from-purple-900 to-black flex items-center justify-center">
                                        <span className="text-white text-xl font-bold">{company.name.substring(0, 2).toUpperCase()}</span>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="bg-white border rounded-lg p-3 shadow-sm flex items-center gap-3">
                            ...
                            <div className="flex gap-0.5">
                                {[1, 2, 3, 4].map(i => (
                                    <div key={i} className="w-6 h-6 bg-[#00C48C] rounded flex items-center justify-center">
                                ...
                                <div className="w-6 h-6 bg-gradient-to-r from-[#00C48C] to-gray-200 rounded flex items-center justify-center relative overflow-hidden">
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <Check size={14} className="text-white z-10" strokeWidth={4} />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {step === 1 && (
                        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                                <div>
                                    <label className="block text-gray-400 text-xs mb-1">Adınız Soyadınız</label>
                                    <div className="font-semibold text-gray-800 text-base">
                                        {user?.userName ? maskName(user.userName) : 'Misafir Kullanıcı'}
                                    </div>
                                </div>

                                <div>
                                    <label className="block text-gray-400 text-xs mb-1">E-posta</label>
                                    <div className="font-semibold text-gray-800 text-base">
                                ...
                                <div>
                                    <label className="block text-gray-400 text-xs mb-1">Telefon Numarası</label>
                                    <div className="font-semibold text-gray-800 text-base">
                        ...
                        <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="flex flex-wrap gap-3 mb-6">
                                {ratingLabels.map((label, index) => (
                                    <div
                                        key={index}
                                        className={`px-4 py-2 rounded-full border text-sm transition-colors ${(hoverRating || rating) === index + 1
                            ...
                            <div className="flex gap-4 mb-8">
                                {[1, 2, 3, 4, 5].map((star) => (
                                    <button
                                        key={star}
                                        className={`w-14 h-14 rounded-lg flex items-center justify-center transition-all duration-200 ${star <= (hoverRating || rating)
                                            ? 'bg-[#FFC107]'
                                            : 'bg-[#E5E7EB]'
                                            }`}
                                    ...
                                    >
                                        <div className="bg-white rounded-md p-1">
                                            <Check
                                                size={20}
                                                className={`stroke-[4px] ${star <= (hoverRating || rating) ? 'text-[#FFC107]' : 'text-[#E5E7EB]'
                                                    }`}
                                            />
                                        </div>
                                    </button>
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex gap-6">
                                    <label className="flex items-center gap-2 cursor-pointer group">
                                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${contactMethod === 'email' ? 'border-[#0D062D] bg-[#0D062D]' : 'border-gray-300 bg-white'}`}>
                                            {contactMethod === 'email' && <Check size={12} className="text-white" />}
                                        </div>
                                        <input
                                            type="radio"
                                            name="contact"
                                            ...
                                            className="hidden"
                                        />
                                        <span className="text-gray-600 group-hover:text-gray-900">E-posta</span>
                                    </label>

                                    <label className="flex items-center gap-2 cursor-pointer group">
                                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${contactMethod === 'phone' ? 'border-[#0D062D] bg-[#0D062D]' : 'border-gray-300 bg-white'}`}>
                                            {contactMethod === 'phone' && <Check size={12} className="text-white" />}
                                        </div>
                                        <input
                                            type="radio"
                                            name="contact"
                                            ...
                                            className="hidden"
                                        />
                                        <span className="text-gray-600 group-hover:text-gray-900">Telefon</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="bg-orange-50 rounded-lg p-4 mb-4 flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mb-2">
                                        Buraya tıklandığında yorum politikası ve topluluk kuralları sözleşmesi çıkacak
                                    </p>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${termsAccepted ? 'border-blue-500 bg-blue-500' : 'border-gray-400 bg-white'}`}>
                                        ...
                                        <input
                                            ...
                                            className="hidden"
                                        />
                                        <span className={`text-xs font-medium ${termsAccepted ? 'text-blue-600' : 'text-gray-400'}`}>
                            ...
                            <div className="bg-[#DCFCE7] border border-[#86EFAC] rounded-xl p-6 flex items-start gap-4 mb-20">
                                ...
                                <div>
                                    ...
                                    <p className="text-[#166534]">
                                        Yorumunuz, ekibimiz tarafından incelenecek ve size en kısa sürede mail yoluyla bilgi verilecek.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex justify-end mt-8">
                        <button
                            ...
                            onClick={() => {
                                ...
                                else if (step === 4 && description.trim() && termsAccepted) handleSubmit();
                        ...
                        >
                            ...
                            <div className="text-xs text-left">
                                {step === 4 ? (isSubmitting ? 'Gönderiliyor...' : 'Gönder') : (step === 5 ? 'Tamamla' : 'Devam etmek için')}<br />
                                "enter"a basın
                            </div>
                        </button>
                    </div>
